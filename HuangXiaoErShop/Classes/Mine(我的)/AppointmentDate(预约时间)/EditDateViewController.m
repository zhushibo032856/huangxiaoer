...
- (void)setNavigationController{
    ...
    self.navigationItem.title = @"编辑预约时间";
...
- (void)creatAutoLayout{
    ...
    NSArray *titleArr = @[@"点击选择开始时间",@"点击选择结束时间"];
   ...
   //         NSLog(@"%@",firstArr);
    ...
    //        NSLog(@"%@",_netArray);
            ...
            for (NSInteger i = 0; i <= YYY - XXX; i ++) {
                [emptyArr insertObject:@"1" atIndex:i];
            }
         [_netArray replaceObjectsInRange:NSMakeRange(XXX, YYY - XXX + 1) withObjectsFromArray:emptyArr];
...
- (void)uploadDateWithDateString:(NSString *)dateString{
 ...
 //   NSLog(@"%@",partner);
    ...
    self.startTF.placeholder = @"";
    self.stopTF.placeholder = @"";
